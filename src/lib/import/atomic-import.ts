import type { ContentStatus, ImportType, Prisma } from "@prisma/client";
...
async function prepareTaxonomy(tx: Prisma.TransactionClient, problems: NormalizedProblem[]) {
  ...
  let sources = await tx.sourceCollection.findMany({ where: { name: { in: sourceNames } } });
  const existingSourceNames = new Set(sources.map((source) => source.name));
  const missingSources = [...sourceInputs.values()].filter((source) => !existingSourceNames.has(source.name));
  if (missingSources.length) {
    await tx.sourceCollection.createMany({ data: missingSources.map((source) => ({
      name: source.name,
    ...
    sources = await tx.sourceCollection.findMany({ where: { name: { in: sourceNames } } });
  }
  const sourceByName = new Map<string, string>();
  for (const source of sources) sourceByName.set(source.name, source.id);

  const topicNames = [...new Set(problems.flatMap((problem) => problem.topics))];
  const topicSlugs = topicNames.map(generateSlug);
  let topics = topicNames.length
    ? await tx.topic.findMany({ where: { OR: [{ name: { in: topicNames } }, { slug: { in: topicSlugs } }] } })
    : [];
  const existingTopicKeys = new Set(topics.flatMap((topic) => [topic.name, topic.slug]));
  const missingTopics = topicNames.filter((name) => !existingTopicKeys.has(name) && !existingTopicKeys.has(generateSlug(name)));
  if (missingTopics.length) {
    await tx.topic.createMany({
      data: missingTopics.map((name) => ({ name, slug: generateSlug(name), description: `Topic được tạo từ import: ${name}` })),
      skipDuplicates: true,
    });
    topics = await tx.topic.findMany({ where: { OR: [{ name: { in: topicNames } }, { slug: { in: topicSlugs } }] } });
  }
...
export async function executeImportPlanAtomically(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const taxonomy = await prepareTaxonomy(tx, bounds.problems);
    ...
    for (const problem of bounds.problems) {
      ...
      const topicIds = problem.topics.map((topic) => taxonomy.topicByKey.get(topic) ?? taxonomy.topicByKey.get(generateSlug(topic)));
