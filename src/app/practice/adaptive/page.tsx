...
export default async function AdaptivePracticePage({ searchParams }: PageProps) {
  ...
  return (
    <div className="grid gap-6">
      <section className="surface-mint rounded-[2rem] p-6 sm:p-8">
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-accent">Luyện tập</p>
      ...
      <form className="surface grid gap-3 rounded-3xl p-5 md:grid-cols-4">
        <label className="grid gap-2 text-sm">
          <span className="font-semibold">Thời lượng</span>
          <select name="minutes" defaultValue={minutes} className="field min-h-11">
            {[10, 20, 30, 45].map((item) => <option key={item} value={item}>{item} phút</option>)}
          </select>
        </label>
        <label className="grid gap-2 text-sm">
          <span className="font-semibold">Trọng tâm</span>
          <select name="focus" defaultValue={focus} className="field min-h-11">
        ...
        <label className="grid gap-2 text-sm">
          ...
          <select name="skill" defaultValue={skill ?? ""} className="field min-h-11">
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {problems.map((problem) => (
          <Link key={problem.id} href={`/problems/${problem.slug}`} className="surface surface-hover rounded-3xl p-6">
