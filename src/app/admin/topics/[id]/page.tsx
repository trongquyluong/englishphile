...
import { updateTopicAction } from "@/app/admin/actions";
...
export default async function AdminTopicDetailPage({ params, searchParams }: PageProps) {
  await requireAdmin();
  ...
  const [topic, allTopics] = await Promise.all([
    ...
    prisma.topic.findMany({ where: { NOT: { id } }, orderBy: { name: "asc" } }),
  ]);
  ...
  return (
    <div className="grid gap-5">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight text-balance">{topic.name}</h1>
        <p className="mt-2 text-sm text-ink-soft">{topic.description ?? "Chưa có mô tả."}</p>
      </div>
      ...
      <section className="surface rounded-lg p-5">
        <h2 className="text-lg font-semibold">Chỉnh sửa topic</h2>
        <form action={updateTopicAction} className="mt-4 grid gap-4 md:grid-cols-2">
          <input type="hidden" name="topicId" value={topic.id} />
          <label className="grid gap-1.5 text-sm font-medium">
            Tên topic
            <input name="name" defaultValue={topic.name} className={inputClass()} />
          </label>
          <label className="grid gap-1.5 text-sm font-medium">
            Slug
            <input name="slug" defaultValue={topic.slug} className={inputClass()} />
          </label>
      ...
      <section className="surface overflow-hidden rounded-lg">
        <div className="border-b border-line px-5 py-4">
          <h2 className="text-lg font-semibold">Problems gắn với topic</h2>
        </div>
        <div className="divide-y divide-line">
          {topic.problemTopics.map(({ problem }) => (
          ...
          {!topic.problemTopics.length ? <p className="p-5 text-sm text-ink-soft">Topic này chưa có problem.</p> : null}
        </div>
      </section>
    </div>
  );
