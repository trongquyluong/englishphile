    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      <section className="grid gap-4">
        {questions.map((question, index) => {
          ...
          return (
            <article key={question.id} className="surface rounded-lg p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                <h2 className="text-sm font-semibold uppercase tracking-[0.08em] text-ink-soft">Câu {index + 1}</h2>
      ...
      <aside className="surface h-fit rounded-lg p-5 lg:sticky lg:top-24">
        ...
        <button
          ...
          className="mt-5 min-h-11 w-full rounded-md bg-foreground px-4 text-sm font-semibold text-background transition-transform duration-150 ease-out active:scale-[0.96] disabled:opacity-60"
        >
