...
Owner-attested Preview evidence dated 2026-07-20 adds a distinct unresolved checkpoint: manual JSON dry-run passed with zero validation errors, then manual commit failed at safe broad stage `problem-create` with database classification. This proves the transaction passed principal revalidation, taxonomy locking/source/topic preparation, and ImportBatch creation, but it does not prove which nested problem operation failed. Taxonomy failure did not reproduce locally, upload-first was deliberately not attempted, and the root cause remains unknown. The uncommitted observability-only correction preserves the original single nested `Problem.create`, including nested Question and ProblemTopic creation, query shape, default-client signature, outer transaction, and rollback behavior. It reports only `problem-nested-create` immediately before that one Prisma operation. The typed Prisma-code allowlist may narrow the database failure class but cannot identify which nested relation caused it; unknown codes/errors remain generic, and authorization failures are not logged as import database failures. The correction neither fixes nor refactors the unknown failure. Keep PR #12 Draft; only after review, commit, and a new isolated Preview deployment may exactly one fresh manual-import probe run before any upload-first retry.
