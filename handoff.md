...
Owner-attested Preview evidence dated 2026-07-23 adds a distinct unresolved checkpoint: manual JSON dry-run passed with zero validation errors, then manual commit failed with `action=import-commit`, `errorClass=database`, `stage=problem-nested-create`, and `prismaCode=unknown`. This confirms the single nested `Problem.create` as the failing application operation, but it cannot identify Prisma's internal parent, Question, or ProblemTopic statement. Taxonomy failure did not reproduce locally, upload-first was deliberately not attempted, and the root cause remains unknown. `unknown` does not prove the absence of a Prisma code: an error outside the earlier allowlist or class-identity failure across bundled/module copies are both possible but unproven.

The uncommitted observability-only correction preserves the original single nested `Problem.create`, including nested Question and ProblemTopic creation, query shape, default-client signature, outer transaction, timeout, and rollback behavior. It continues to report only `problem-nested-create` immediately before that one Prisma operation. The fixed code allowlist now includes `P2024` and `P2028`, and guarded own-property descriptors permit exact bundled-copy identities without reading metadata or arbitrary fields. A separate fixed `prismaErrorKind` reports only `known-request`, `unknown-request`, `initialization`, `validation`, `rust-panic`, `not-prisma`, or `unknown`. These added codes and bundled recognition remain diagnostic hypotheses until a later safe signal identifies one; even a `P2002` or `P2003` cannot identify which nested relation caused it. No timeout, exact duration, write, query, authorization, or import behavior changed. Authorization failures remain outside the database-failure logger. The correction neither fixes nor refactors the unknown failure. Keep PR #12 Draft; only after review, commit, and a new isolated Preview deployment may exactly one fresh manual-import probe run before any upload-first retry.
