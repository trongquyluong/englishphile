...
Latest owner-attested Preview evidence dated 2026-07-23 records that manual JSON dry-run passed with zero validation errors and manual commit passed principal revalidation, ImportBatch creation, taxonomy advisory locks, source lookup/creation, and topic lookup/creation before failing at the single nested `Problem.create`. The safe signal was `action=import-commit`, `errorClass=database`, `stage=problem-nested-create`, `prismaErrorKind=known-request`, and `prismaCode=P2028`. Prisma defines P2028 as a Transaction API error. The callback had no per-call timeout and therefore used Prisma's documented 5,000 ms interactive-transaction default. Because raw Prisma message and metadata were intentionally not logged, transaction expiry is the leading evidence-based hypothesis rather than a proven internal P2028 subtype. Upload-first remains untested.
...
`prisma validate`, `prisma generate`, TypeScript typecheck, ESLint, and the optimized Next.js build pass. Build page collection used explicit synthetic unreachable process configuration and emitted only fixed generic database signals. The focused timeout/observability run passes 30 cases across 3 files. The guarded PGlite bootstrap verified a fresh owned engine, applied and checked all 16 immutable migrations, and passed all 8 isolated cases. Both required ordinary default-parallel test runs pass 459 cases and skip only the 8 gated cases. An additional two-process stress invocation caused the existing valid-XLSX case to cross its unchanged five-second timeout in one process; the ordinary required runs immediately passed, and no test or global timeout was changed. `npm audit` and `npm audit --omit=dev` complete with nonzero status and each report six vulnerability instances (three moderate and three high) across the existing brace-expansion, PostCSS/Next, sharp, and UUID/ExcelJS advisory paths. No audit fix, upgrade, dependency, or lockfile edit was performed. Prisma also reports its existing package-configuration deprecation warning.
