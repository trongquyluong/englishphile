...
The exact historical Preview failure operation cannot be recovered from the available generic `action=prisma`/`errorClass=database` signal. The original `e8e3a6752c74055f973af3d47a2135bc52ed98b9` missing-taxonomy path succeeded on the fresh isolated engine, so the correction pass does not claim that create/re-fetch visibility or an inherent five-second timeout was the proven Preview cause. The transaction now acquires one deterministic transaction-scoped advisory-lock set for source names and topic slugs, creates missing taxonomy with `createManyAndReturn`, and uses returned IDs without re-fetching. Source names remain non-unique in the schema; the advisory lock deliberately serializes cooperating import commits without pretending an upsert-by-name is database-enforced. A failed transaction logs only a static stage and safe error class. A future isolated Preview retry can therefore distinguish principal, pack, batch, taxonomy lookup/create, nested content, finalization, and reconciliation failures without logging imported values or raw Prisma errors. PR #12 must remain Draft and the operational blocker must not be declared cleared until that retry succeeds or supplies the new safe stage evidence.
...
After the PR #12 correction, the suite contains 447 cases across 39 files: 305 runtime production helper/handler/action/page/orchestrator cases with mocked collaborators where stated, 15 simulations (including five executable integration-runner guard cases), 119 static source/structure checks, and 8 gated isolated PostgreSQL-engine integration cases. The default command runs 439 cases and reports the 8 explicitly gated integration cases as skipped; the integration command runs those 8 cases separately against the isolated engine. The ImportCenter message wiring case is explicitly static; the upload route status cases invoke the production handler. Final command outcomes for this correction are recorded in the review report rather than treated as Preview or Production evidence.
