...
**Current Phase 1D-A reassessment:** High severity and directly exploitable before remediation. The original narrow diagnostic-result-page rendering claim became stale, but the broader exposure remained real: answer-bearing Prisma question data crossed learner Server/Client boundaries before submission, and canonical-answer-derived feedback crossed practice API boundaries after submission. Diagnostic finalization also duplicated answer material in stored recommendation metadata. The repository implementation is remediated, PR #10 is merged, isolated Preview sentinel boundaries passed, and selected Production structural, response-shape, authorization, regression, and runtime-log checks passed.

**Phase 1D-A disposition (2026-07-17):** Remediated in repository code and merged through PR #10 at `ce8c9bfc7b4f2135158960e11ab486dd7fffbb59`, which owner-attested evidence identifies as the canonical READY Production deployment. The implementation replaces learner presentation, submission, and diagnostic-result boundaries with positive allowlist DTOs; fixed generic feedback replaces answer-bearing checker feedback; diagnostic scoring remains server-only; newly finalized diagnostic metadata no longer duplicates `correctAnswer` or answer-bearing feedback; and result reads require current-user ownership, finalized status, and non-null completion before returning aggregate data. Historical `recommendationJson` rows were not rewritten and are parsed through an allowlist. Learner analytics, wrong-question review, and contest result rendering also stop displaying canonical answers or stored answer-bearing feedback. Admin editor/preview mappings remain explicitly separate and retain full answer access. No schema change or migration exists for Phase 1D-A.

The focused evidence and limitations are recorded in `docs/SECURITY_PHASE_1D_A_REPORT.md`. Repository evidence includes 366 local tests: 242 runtime/helper/handler/action/page tests, 8 simulations, 116 static checks, and 0 PostgreSQL integration tests. The production-page runtime test mocks `requireUser`, result/recommendation selectors, and redirect control flow; it proves that the page scopes results only with `requireUser().id`, including stored-ADMIN, owner-shaped STUDENT, and ordinary-STUDENT fixtures. It is not an end-to-end cookie/session test and does not independently verify `OWNER_EMAIL` configuration matching; existing content-admin policy tests remain that evidence. The owner-attested Preview and Production observations below are operational evidence, not repository tests.
...
The sentinels were synthetic and non-sensitive; their exact values are not recorded. Browser/RSC inspection was owner-attested operational evidence, not a repository test, and browser automation is not claimed. No identity, cookie, deployment ID, infrastructure hostname, database identifier, or protected URL is recorded. Synthetic fixture cleanup was not reported. At this historical Preview checkpoint PR #10 was Draft/open; it subsequently merged as recorded below.

**Owner-attested selected Production evidence (2026-07-17):** PR #10 is `MERGED`, and merge commit `ce8c9bfc7b4f2135158960e11ab486dd7fffbb59` became the canonical READY Production deployment. The deployment source was confirmed separately because the Vercel CLI did not report commit metadata. Health returned HTTP 200 with the database connected. A missing-Origin submission POST returned 403 at the origin guard, while a separate same-origin unauthenticated submission returned 401 at authentication. Existing published-problem HTML/RSC contained none of the tested forbidden answer-key tokens. Authenticated single-problem and random-practice responses contained only expected safe fields and fixed generic feedback. Diagnostic start/result remained learner-safe and aggregate-only; a nonexistent/incomplete own attempt followed unavailable behavior. Analytics, skill analytics, wrong-question review, and contest-result review did not expose canonical-answer fields. Authorized admin preview retained answer/explanation access. Owner sign-out/sign-in and admin access, ordinary-`STUDENT` admin denial, independent-practice write/persistence, and basic contest/diagnostic/Writing regression smoke passed. Checked runtime logs reported no runtime errors or sensitive values, and Git branch/status was `main`/clean at the operational checkpoint.

Production used no synthetic sentinel fixtures; therefore Production sentinel verification is not claimed. Production HTML/RSC evidence was structural forbidden-token inspection and is weaker than isolated Preview sentinel evidence. No deployment ID, infrastructure hostname, protected URL, or provider-internal identifier is recorded. This evidence does not establish comprehensive browser automation, comprehensive security testing, hostile-origin testing, concurrency, rollback, timeout, exactly-once behavior, PostgreSQL integration, database-row inspection, or historical-row cleanup. Basic contest/diagnostic/Writing checks were regression smoke only, and runtime-log evidence is limited to the checked deployment/time window. No Production schema migration was needed or run for Phase 1D-A. H-11 at-rest remediation and dependency-advisory resolution are not claimed.
...
The local remediation alone did not constitute browser or deployment evidence; the later owner-attested Preview and Production evidence above verifies only the explicitly tested boundaries.
...
20. **H-10: Maintain the deployed remediation** — Repository remediation is merged and canonically deployed; isolated Preview sentinel checks and selected Production checks passed, while PostgreSQL integration and ongoing monitoring remain outstanding
...
| H-10 diagnostic/learner answer data | Remediated, merged, canonically deployed, and selected boundaries verified | PR #10 merged at `ce8c9bfc7b4f2135158960e11ab486dd7fffbb59`; isolated Preview sentinel checks and selected Production structural/response/authorization/regression/log checks passed; historical rows remain unchanged, PostgreSQL integration remains Test debt, and ongoing monitoring is required |
...
| H-10 diagnostic/learner answer data | Unresolved at this historical Phase 1C-A checkpoint | Superseded by the current Phase 1D-A merged, deployed, isolated-Preview, and selected-Production disposition above |
...
| H-10 diagnostic/learner answer data | Unresolved at this historical Phase 1C-B checkpoint | Superseded by the current Phase 1D-A merged, deployed, isolated-Preview, and selected-Production disposition above. |
